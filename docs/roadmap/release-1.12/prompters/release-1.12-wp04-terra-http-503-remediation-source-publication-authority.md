# GPT-5.6 Terra — Release 1.12 WP04 HTTP 503 Remediation Source Publication Authority
...
- **GPT-5.6 Terra** — PRIMARY: publish the validated one-path HTTP 503 retry remediation.
...
terminal statuses preserved: 400,401,403,409,500,502 and other non-governed failures
malformed payload: terminal
wrong RunId: terminal
bounded timeout: terminal
synthetic cases: 12 passed
Windows PowerShell AST: 0 errors
Windows PowerShell baseline: 5.1.26100.9444
sanitized attempt/status output: present
RandomNumberGenerator.Fill active references: 0
RandomNumberGenerator.Create references: 1
GetBytes references: 1
direct SQLite evidence scan: 0
active Kudu-VFS evidence scan: 0
Gitleaks: pass
git diff --check: pass
...
`RELEASE 1.12 WP04 — TERRA HTTP 503 RETRY REMEDIATION PUBLICATION AUTHORITY: READY`
...
```text
HEAD = da5108a6ad73b6e1a23df2b9df3d170d1658886a
...
- retry-policy static proof that only `404` and `503` are retryable;
- synthetic retry/terminal regression suite;
- RNG compatibility checks;
- token-disclosure scan;
- direct SQLite scan;
...
`RELEASE 1.12 WP04 — HTTP 503 SOURCE PUBLICATION PRECHECK: PASS`
...
Create exactly one commit containing only this helper remediation.

Use a concise WP04-specific message identifying bounded HTTP 503 readiness retry behavior.
...
`RELEASE 1.12 WP04 — HTTP 503 REMEDIATION SOURCE COMMIT: PASS`
...
`RELEASE 1.12 WP04 — HTTP 503 REMEDIATION SOURCE PUSH: PASS`
...
runtime image payload source remains application/container payload from earlier source
...
`RELEASE 1.12 WP04 — HTTP 503 REMEDIATION IMAGE PRESERVATION: PASS`
...
The prior failed initialize RunId remains forbidden for reuse:
...
`RELEASE 1.12 WP04 — HTTP 503 SOURCE PUBLICATION MUTATION AUDIT: PASS`

`RELEASE 1.12 WP04 — TERRA AZURE QUALIFICATION RESUME AUTHORITY: READY`

`RELEASE 1.12 WP04 — TERRA HTTP 503 REMEDIATION SOURCE PUBLICATION COMPLETE`

Blocked:

`RELEASE 1.12 WP04 — HTTP 503 REMEDIATION SOURCE PUBLICATION: BLOCKED`
