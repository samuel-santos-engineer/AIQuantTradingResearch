...
The WP04 `SemaphoreSlim` remains effective inside each child. Multiple Streamlit sessions can launch children concurrently, so the implementation must add no distributed service and must not claim process-local coalescing is global. Atomic cache replacement prevents partial records. Before provider acquisition, a minimal local lock keyed by the canonical cache fingerprint is required only if measurement or tests demonstrate duplicate concurrent misses violate the bounded-acquisition contract; it must be a filesystem/process primitive under the existing cache directory, have a short bounded lease, recover from abandoned ownership, and never be a schema-backed lock. If atomic cache behavior and bounded duplicate calls are accepted by tests, no additional lock is needed.

## F1 and deployment fit

The design is compatible with Linux F1: no listener, background poller, managed cache, database, broker, CDN, or paid service is added. Startup cost occurs only on the three governed interaction classes, memory is released at child exit, cache reuse suppresses provider calls, and the existing `/home` persistence boundary remains authoritative. The existing entrypoint continues starting Worker and Streamlit; implementation must package the bridge executable already produced by the Worker build. Docker/startup changes are not authorized by this ADR; if packaging cannot expose the existing executable without Docker mutation, implementation stops for a separate deployment authority.
