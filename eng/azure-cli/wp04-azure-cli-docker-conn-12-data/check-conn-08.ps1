...
Write-Host '=== REMOVE ONLY THE OWNED SECRET SETTING ==='
az webapp config appsettings delete --resource-group $rg --name $app `
  --setting-names TWELVE_DATA_API_KEY --output none
Write-Host "WP04_MISSING_SECRET_SETTING_REMOVE_EXIT_CODE=$LASTEXITCODE"
if ($LASTEXITCODE -ne 0) { throw 'Secret-setting removal failed.' }

Write-Host '=== POLL HEALTH AFTER SETTING CHANGE ==='
$healthy = $false
for ($attempt = 1; $attempt -le 30; $attempt++) {
    Write-Host "WP04_MISSING_SECRET_HEALTH_POLL_ATTEMPT=$attempt"
    try {
        $health = Invoke-RestMethod -Uri "$baseUrl/healthz" -TimeoutSec 20
        if ($health.status -eq 'ok') { $healthy = $true; break }
    } catch {
        Write-Host "WP04_MISSING_SECRET_HEALTH_POLL_ERROR=$($_.Exception.Message)"
    }
    Start-Sleep -Seconds 10
}
Write-Host "WP04_MISSING_SECRET_HEALTHY=$healthy"
if (-not $healthy) { throw 'Host was not healthy after secret removal.' }

Write-Host '=== VERIFY PRESENCE BOOLEAN ONLY ==='
$diagnostics = Invoke-RestMethod -Uri "$baseUrl/diagnostics" -TimeoutSec 30
$diagnostics | ConvertTo-Json -Compress
Write-Host 'WP04_MISSING_SECRET_DIAGNOSTICS_EXIT_CODE=0'
if ($diagnostics.secretPresent -ne $false) { throw 'Secret remains present after removal.' }
