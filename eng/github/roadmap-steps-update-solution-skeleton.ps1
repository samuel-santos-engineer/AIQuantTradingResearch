...
| ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------- | ------------ | ------------- |
| 01 — Repository Preflight            | Inspect repository structure, architecture guidance, build assets, engineering scripts, SDK configuration, existing projects and constraints before making implementation changes.                 | P0       | 0.8     | architecture | research      |
| 02 — Root Solution                   | Create the root `AIQuantTradingResearch.slnx` solution, or the repository-supported equivalent, and establish the solution as the container for production and test projects.                      | P0       | 0.8     | architecture | feature       |
| 03 — Production Projects             | Create the minimal Domain, Application, Infrastructure and Worker projects under `src/`, removing template/sample code and avoiding speculative business implementation.                           | P0       | 0.8     | architecture | feature       |
| 04 — Project References              | Configure project references according to the approved dependency direction and ensure forbidden dependencies cannot be introduced through the initial project graph.                              | P0       | 0.8     | architecture | feature       |
| 05 — Root Build Configuration        | Validate and align `global.json`, `Directory.Build.props`, `Directory.Packages.props`, `.editorconfig` and related root configuration with the new solution without replacing existing governance. | P0       | 0.8     | infra        | feature       |
| 06 — Minimal Worker Host             | Create the minimal Worker executable using the .NET host model, configuration and composition root without introducing market data, storage, trading, AI/ML or other later-release capabilities.   | P0       | 0.8     | architecture | feature       |
| 07 — Dependency Registration         | Establish `AddApplication` and `AddInfrastructure` composition boundaries so the Worker can compose the platform without leaking infrastructure concerns into inner layers.                        | P0       | 0.8     | architecture | feature       |
| 08 — Test Projects                   | Create Domain, Application, Infrastructure and Architecture test projects and connect each project only to the production layers required by its testing responsibility.                           | P0       | 0.8     | tests        | feature       |
| 09 — Architecture Tests              | Implement executable architecture tests that prevent Domain → outer-layer, Application → Infrastructure/Worker and Infrastructure → Worker dependencies.                                           | P0       | 0.8     | tests        | tests         |
| 10 — Solution Organization           | Add all production and test projects to the root solution and organize them consistently so the physical solution remains easy for engineers and coding agents to navigate.                        | P1       | 0.8     | architecture | feature       |
| 11 — Engineering Scripts Integration | Integrate the new solution with existing `eng/` restore, build, clean, format, test and verify automation while avoiding duplicated build logic or unnecessary new tooling.                        | P0       | 0.8     | devops       | enhancement   |
| 12 — Documentation Alignment         | Update only documentation affected by the physical skeleton so project paths, solution format, engineering commands and implemented repository state accurately reflect reality.                   | P1       | 0.8     | architecture | documentation |
| 13 — Full Skeleton Validation        | Execute the complete restore → build → test → format → architecture-test → repository-verify sequence and capture reproducible evidence that the skeleton works from the repository root.          | P0       | 0.8     | tests        | tests         |
| 14 — GitHub Integration              | Execute the Solution Skeleton through the governed issue → branch → implementation → validation → commit → pull request → review workflow and preserve implementation evidence.                    | P1       | 0.8     | devops       | feature       |
| 15 — Release Acceptance Review       | Review structure, dependencies, host composition, tests, engineering scripts, documentation and validation evidence and determine whether Release 0.8 satisfies its Definition of Done.            | P0       | 0.8     | architecture | research      |
"@

# Configuration metrics pointing to your target roadmap name
$projectName = "AIQuantTradingResearch Engineering Roadmap"
$milestone = "Phase 2 - Release 0.8: Solution Skeleton"
$projectWebNumber = 2
$owner = "@me"
...
foreach ($line in $lines) {
  ...
  if ($line -notmatch '^\|\s*\d{2}\s+[—-]\s+.*\|') { continue }
